...
test("inferSimplices can emit a soft cluster from three mutually strong relations", () => {
  ...
  const softCluster = simplices.find(
    (simplex: { inferredSignals?: string[]; nodes: string[] }) =>
...
test("model analysis summary reports graph-level metrics", () => {
  ...
  assert.ok(
    summary.maxSimplexCentralityNodeId && ["a.md", "b.md", "c.md"].includes(summary.maxSimplexCentralityNodeId),
  );
