...
function Ji(r, t, e) {
  ...
  switch (Math.floor(Math.random() * 4)) {
    case 0:
      return { px: De(-s, s), py: -o };
    case 1:
      return { px: s, py: De(-o, o) };
    case 2:
      return { px: De(-s, s), py: o };
    default:
      return { px: -s, py: De(-o, o) };
...
var Re = class {
  ...
  updateHyperedge(t, e) {
    ...
    return (
      ...
      e.weight !== void 0 && (i.weight = Ne(e.weight)),
    ...
    tick(t, e, n, i, s = []) {
      ...
      for (let g = 0; g < t.length; g++)
        for (let f = g + 1; f < t.length; f++) {
          ...
          if (N > 0) {
            let K = Math.min(1.6, N),
              pe = Math.max(this.MIN_NODE_SEPARATION * 1.05, this.SPARSE_EDGE_LENGTH * (1.08 - K * 0.2)),
              G = (S - pe) * this.COHESION * 0.16 * (1 + N * 0.9),
              ge = pe * 0.68,
              B = Math.max(0, ge - S) * this.COHESION * 0.28 * (1 + N * 1.1);
      ...
      t.forEach((g) => {
        ...
        if (
          ...
          let S = Qs(g.id) * Math.PI * 2,
            N = Math.max(this.SPARSE_EDGE_LENGTH * 2.8, 320),
            I = Math.cos(S) * N,
            L = Math.sin(S) * N;
...
function rr(r, t, e, n) {
  ...
  else if (si(o)) (Nt(m, o[0], o[o.length - 1], n), m.fill());
  else
    for (let y = 0; y < s.length; y++) {
      for (let b = y + 1; b < s.length; b++)
        (Nt(m, { x: s[y].px, y: s[y].py }, { x: s[b].px, y: s[b].py }, n), m.fill());
...
function ur(r, t, e, n, i, s) {
  ...
  let c = l.reduce((S, N) => ({ x: S.x + N.px, y: S.y + N.py }), { x: 0, y: 0 });
  ...
  let b = Math.sqrt(l.reduce((S, N) => S + (N.px - c.x) ** 2 + (N.py - c.y) ** 2, 0) / l.length);
  if (y || b < 200) {
    ...
    (c.forEach(([m, v]) => {
      ...
      o.forEach((m) => {
        ...
        let v = m.id === i.hoveredNodeId,
          ...
          [S, N, I] = b ? F(this.model, b) : [136, 135, 128];
        v &&
          (t.beginPath(), t.arc(m.px, m.py, 15, 0, Math.PI * 2), (t.fillStyle = `rgba(${S},${N},${I},0.10)`), t.fill());
        ...
        (t.beginPath(),
          m.isVirtual
            ? (t.arc(m.px, m.py, ue, 0, Math.PI * 2),
              (t.strokeStyle = `rgba(${S},${N},${I},${pe})`),
              (t.lineWidth = 1.5),
              t.stroke())
            : (t.arc(m.px, m.py, ue, 0, Math.PI * 2), (t.fillStyle = `rgba(${S},${N},${I},${pe})`), t.fill()),
          m.isPinned &&
            (t.beginPath(),
...
var It = class extends ee.Modal {
  onOpen() {
    ...
    ([
      ...
      this.addPrompt("Combined discovery prompt", Di),
...
var qe = class extends T.ItemView {
  ...
  renderMeasure(e, n, i, s, o) {
    ...
    (l.createSpan({ cls: "simplicial-measure-name", text: n }),
      ...
      s && a.createDiv({ cls: "simplicial-measure-reading", text: s }));
  }
  renderExplanationCard(e, n) {
    ...
    if (
      (o.createDiv({ cls: "simplicial-explanation-headline", text: s.headline }),
      o.createDiv({ cls: "simplicial-explanation-tension", text: s.tension }),
      o.createDiv({ cls: "simplicial-explanation-prompt", text: s.prompt }),
      s.signals.length > 0)
    ) {
...
var Se = {
  ...
  Ze = class extends he.ItemView {
    ...
    render() {
      ...
      if (
        (dt.forEach((i) => {
          ...
          (s.createSpan({ cls: `simplicial-kernel-swatch is-${i}` }),
            s.createSpan({ cls: "simplicial-kernel-name", text: Se[i].title }),
            s.createSpan({ cls: "simplicial-kernel-claim", text: Se[i].claim }));
        }),
        this.runs.length === 0)
      ) {
      ...
      (i.createDiv({ cls: "simplicial-panel-section-label", text: n.label }),
        n.results.forEach((o) => {
          let a = i.createDiv({ cls: "simplicial-dynamics-row" }),
            l = a.createDiv({ cls: "simplicial-measure-head" });
          (l.createSpan({ cls: `simplicial-kernel-name is-${o.kernel}`, text: Se[o.kernel].title }),
            l.createSpan({
    ...
    renderCompetingRhythms(e) {
      ...
      (s.createDiv({ cls: "simplicial-panel-section-label", text: "Competing rhythms" }),
...
var wr = ["manual", "folder", "tag", "query", "moc"],
  Je = class extends D.ItemView {
    ...
    renderSuggestions(e, n) {
      ...
      (s.createDiv({ cls: "simplicial-panel-section-label", text: "Suggested starting cover" }),
        s.createDiv({
          cls: "simplicial-measure-reading",
          text: i.length
    ...
    renderCreator(e, n) {
      ...
      let s = "",
        ...
        a = "",
        l = new Set();
      (new D.Setting(i).setName("Name").addText((d) => d.onChange((h) => (s = h.trim()))),
        new D.Setting(i).setName("Source").addDropdown((d) => {
          (wr.forEach((h) => {
      ...
      (c.forEach((d) => {
        new D.Setting(u).setName(this.relationLabel(d)).addToggle((h) => {
          h.onChange((p) => (p ? l.add(d) : l.delete(d)));
        });
      }),
        new D.Setting(i).addButton((d) => {
          (d.setButtonText("Add context").setCta(),
            d.onClick(async () => {
              if (!s) {
                new D.Notice("Give the context a name first.");
                return;
              }
              let h = (o === "folder" || o === "tag") && a ? St(this.app, this.model, o, a, n.contexts) : null,
                p = l.size > 0 ? [...l] : (h?.relations ?? []);
              if (p.length === 0) {
                new D.Notice("Select at least one relation, or use a folder/tag that matches one.");
                return;
              }
    ...
    renderContext(e, n, i) {
      ...
      (a.createEl("strong", { text: s.name }),
        a.createSpan({ text: `${s.source}${s.definition ? ` \xB7 ${s.definition}` : ""}` }));
      ...
      if (
        ...
        (p.createDiv({ cls: "simplicial-panel-section-label", text: `Readings of ${A(this.compareNodeId)}` }),
          pn(this.model, n, h, this.compareNodeId).forEach((g) => {
            p.createDiv({ text: `${g.contextName}: ${g.role} \xB7 ${g.provenance}` });
          }));
      }
      (this.renderContextRestructuring(o, n, s.id),
        new D.Setting(o).addButton((h) => {
    ...
    renderAudit(e, n) {
      ...
      (s.length === 0 && i.createDiv({ cls: "simplicial-measure-reading", text: "No accepted refinements yet." }),
        s.forEach((o) =>
          i.createDiv({
            cls: "simplicial-measure-reading",
